...
function initPjax() {
    ...
    a.prototype._initOnResizeCallbacks = function () {
        ...
        var f = $($.parseHTML(b.match(/<body[^>]*>([\s\S.]*)<\/body>/i)[0], document, !0)), g = f.filter("script[src]").add(f.find("script[src]")), h = f.filter('script[type="text/template"]').add(f.find('script[type="text/template"]')), i = $("script[src]"), j = $('script[type="text/template"]');
        h.each(function () {
            var a = this.id, b = j.filter(function () {
                return this.id === a
            });
            if (!b.length) {
        ...
        g.each(function () {
            var a = this.src, b = i.filter(function () {
                return this.src === a
            });
            if (!b.length) {
                var c = document.createElement("script");
...
function testData(a, b) {
    ...
    return stream_layers(a.length, b, .1).map(function (b, c) {
        return {
            key: a[c], values: b.map(function (a, b) {
                return {x: g + a.x * d * h, y: Math.floor(100 * a.y)}
            })
        }
    })
}
...
var $lime = "#8CBF26", $red = "#f25118", $redDark = "#d04f4f", $blue = "#4e91ce", $green = "#3ecd74", $orange = "#f2c34d", $pink = "#E671B8", $purple = "#A700AE", $brown = "#A05000", $teal = "#4ab0ce", $gray = "#666", $white = "#fff", $textColor = $gray, chartsOff = !1;
chartsOff && (nv.addGraph = function () {
}), COLOR_VALUES = [$red, $orange, $green, $blue, $teal, $redDark], window.colors = function () {
